...
const updatingProgram = async () => {	
  console.log("cureent",currentProgram.value)
...
<template>
	<div class="container-full mt-5 p-3">
			...
			<div class="row">
				<div class="col-12 p-2">
					...
					<router-link to="/programlist" custom v-slot="{ navigate }">
						<button type="button" class="btn btn-primary float-end mx-2" data-nav="ngo.program"
							@click="navigate" role="link">
							Cancel
						</button>
					</router-link>
					<button type="submit" class="btn btn-primary float-end mx-2" data-nav='ngo.programs'>Save</button>
				</div>
			</div>
					...
					<div class="col-4">
						...
						<select id="level" class="form-select"
						  ...
						  <option value="" selected>Donor</option>
							<option v-for="item in donors" :value="item.name">{{ item.name }}</option>
						</select>
					</div>

					<div class="col-4">
						<label for="exampleFormControlInput1">Supplement</label>
						<select id="level" class="form-select" v-model="currentProgram.supplement_name">
							<option value="" selected>Supplement</option>
							<option v-for="item in supplements.data" v-bind:value="item.name">{{ item.name }}</option>
						</select>
					</div>
